...
const regex = /<div className="flex w-full overflow-x-auto gap-2 md:gap-3 mb-8 pb-4 border-b border-gray-100 hide-scrollbar"[\s\S]*?<\/div>\r?\n\s*\r?\n\s*<div className="grid/g;

const replacement = `<div className="flex w-full overflow-x-auto gap-3 md:gap-4 mb-10 pb-4 hide-scrollbar snap-x" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
				{parts.map((p) => {
					...
					return (
						<button
							...
							className={\`group flex items-center gap-2.5 px-4 md:px-5 py-2.5 md:py-3 rounded-2xl transition-all duration-300 font-bold text-[13px] md:text-[14px] whitespace-nowrap shrink-0 snap-center \${isActive ? colors.activeBg : colors.bg}\`}
						>
