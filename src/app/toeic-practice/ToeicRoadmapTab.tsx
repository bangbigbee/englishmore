...
export default function ToeicRoadmapTab({ level, score, onPracticeClick, onTabClick }: { level: string | null, score: string | null, onPracticeClick: (path: string) => void, onTabClick: (tab: string) => void }) {
    ...
    if (!level) {
        return (
            <div className="bg-white rounded-3xl p-8 md:p-12 text-center border border-slate-200 shadow-sm flex flex-col items-center">
                ...
                <h2 className="text-2xl font-black text-primary-900 mb-4">Lộ Trình Học Tập Cá Nhân Hóa</h2>
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {score && (
                        <div className="mt-6 flex flex-wrap items-center gap-4">
                            <span className="text-sm font-semibold text-slate-700 bg-white/80 px-5 py-2.5 rounded-2xl shadow-sm border border-slate-200/60 flex items-center">
                                Điểm thi đầu vào: <span className="font-black text-primary-900 ml-2 text-lg">{score}</span>
                            </span>
            ...
            {/* Tasks */}
            <div className="mb-6">
                <h3 className="text-lg font-black text-primary-900 mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {currentRoadmap.tasks.map((task, idx) => (
                        <div key={idx} className={`bg-white rounded-2xl p-5 border border-slate-200 shadow-sm transition-all group flex flex-col relative overflow-hidden hover:border-primary-900/30 hover:shadow-md`}>
                            ...
                            <h4 className="font-bold mb-4 text-slate-800 transition-colors group-hover:text-primary-900">
                                {task.title}
                            </h4>
                            ...
                            <button
                                ...
                                className="mt-auto w-full py-2.5 rounded-xl font-bold text-sm border transition-all cursor-pointer bg-slate-50 text-primary-900 border-slate-200 group-hover:bg-primary-900 group-hover:text-primary-50 group-hover:border-primary-900 flex items-center justify-center gap-1.5"
                            >
            ...
            <div className="mt-8 text-center">
                <button 
                    ...
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-900 hover:text-primary-800 transition-colors cursor-pointer group"
                >
