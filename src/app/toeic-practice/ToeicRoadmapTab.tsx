...
export default function ToeicRoadmapTab({ level, score, onPracticeClick, onTabClick }: { level: string | null, score: string | null, onPracticeClick: (path: string) => void, onTabClick: (tab: string) => void }) {
    ...
    if (!level) {
        return (
            <div className="bg-white dark:bg-slate-900/50 rounded-3xl p-8 md:p-12 text-center border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center">
                <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800/80 rounded-full flex items-center justify-center mb-6">
                    <span className="text-4xl">🗺️</span>
                </div>
                <h2 className="text-2xl font-black text-primary-900 dark:text-primary-400 mb-4">Lộ Trình Học Tập Cá Nhân Hóa</h2>
                ...
                <button 
                    ...
                    className="bg-primary-900 hover:bg-primary-800 text-primary-100 px-6 py-3 rounded-xl font-bold transition-all shadow-md active:scale-95 cursor-pointer"
                >
    ...
    const roadmapData: Record<string, {
        ...
        'BEGINNER': {
            ...
            color: 'text-primary-900 dark:text-primary-400',
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Header */}
            <div className={`rounded-3xl p-8 mb-8 border shadow-sm relative overflow-hidden ${currentRoadmap.bg} border-white dark:border-primary-900/30`}>
                ...
                <div className="relative z-10">
                    ...
                    {score && (
                        <div className="mt-6 flex flex-wrap items-center gap-4">
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 bg-white/80 dark:bg-slate-900/60 px-5 py-2.5 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700 flex items-center">
                                Điểm thi đầu vào: <span className="font-black text-primary-900 dark:text-primary-400 ml-2 text-lg">{score}</span>
                            </span>
                            ...
                            >
                                <div className="absolute inset-0 bg-gradient-to-r from-primary-600 to-blue-500 opacity-80 group-hover:opacity-100 transition-opacity" />
                                <div className="absolute -inset-1 bg-gradient-to-r from-primary-600 to-blue-500 blur-sm opacity-40 group-hover:opacity-60 transition-opacity" />
                                <span className="relative flex items-center gap-2">
                                    ...
                                    Lộ Trình Của Tôi
                                    <span className="text-secondary-400 ml-1 group-hover:translate-x-1 transition-transform">&rarr;</span>
                                </span>
                            </button>
                        </div>
                    )}
            ...
            {/* Tasks */}
            <div className="mb-6">
                <h3 className="text-lg font-black text-primary-900 dark:text-primary-400 mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {currentRoadmap.tasks.map((task, idx) => (
                        <div key={idx} className={`bg-white dark:bg-slate-900/50 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm transition-all group flex flex-col relative overflow-hidden hover:border-primary-900/30 dark:hover:border-primary-500/30 hover:shadow-md`}>
                            ...
                            <h4 className="font-bold mb-4 text-slate-800 dark:text-slate-200 transition-colors group-hover:text-primary-900 dark:group-hover:text-primary-400">
                                {task.title}
                            </h4>
                            ...
                            <button
                                ...
                                className="mt-auto w-full py-2.5 rounded-xl font-bold text-sm border transition-all cursor-pointer bg-slate-50 dark:bg-slate-800 text-primary-900 dark:text-primary-400 border-slate-200 dark:border-slate-700 group-hover:bg-primary-900 dark:group-hover:bg-primary-600 group-hover:text-primary-50 dark:group-hover:text-white group-hover:border-primary-900 dark:group-hover:border-primary-600 flex items-center justify-center gap-1.5"
                            >
                            ...
                            {task.isPremium && (
                                <div className="absolute top-3 right-3 text-[9px] font-bold tracking-widest bg-primary-100 text-primary-700 px-1.5 py-0.5 rounded">
                                    PRO
                                </div>
                            )}
                        </div>
            ...
            <div className="mt-8 text-center">
                <button 
                    ...
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-900 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 transition-colors cursor-pointer group"
                >
                    <svg className="w-4 h-4 text-secondary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
