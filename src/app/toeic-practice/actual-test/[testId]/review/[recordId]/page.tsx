    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-800">
                <div className="animate-pulse font-bold text-xl text-indigo-600">Đang tải dữ liệu Review...</div>
            </div>
        );
    }
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-slate-50 text-slate-800">
            <div className="flex flex-col h-screen overflow-hidden">
                {/* Header */}
                <div className="flex flex-row items-center justify-between px-2 py-3 md:p-4 gap-1 md:gap-4 bg-white shadow-sm border-b border-indigo-100">
                    <div className="flex items-center gap-1.5 md:gap-4 flex-shrink-0">
                        <h1 className="font-black text-sm md:text-xl max-w-[200px] md:max-w-none truncate text-indigo-900" title={testData.title}>REVIEW: {testData.title}</h1>
                        <span className="px-1.5 md:px-3 py-0.5 md:py-1 rounded-full text-[9px] md:text-xs font-bold whitespace-nowrap bg-indigo-100 text-indigo-700">
                            XEM CHỮA LỖI
                        </span>
                    </div>

                    <div className="flex items-center gap-1.5 md:gap-4 flex-1 justify-end">
                        <button onClick={() => setIsSheetOpen(!isSheetOpen)} className="lg:hidden px-2 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-sm font-bold flex items-center gap-1 md:gap-2 whitespace-nowrap flex-shrink-0 bg-blue-100 text-blue-700">
                ...
                <div className="flex flex-1 overflow-hidden">
                                ...
                                <div key={pIdx} id={`part-${partInfo.part}`} className="mb-12">
                                    ...
                                    <div className="space-y-10">
                                        {partInfo.questions.map((q: any, qIdx: number) => {
                                            ...
                                            return (
                                            <div key={q.id} id={`q-${qNum}`} className={`p-6 rounded-2xl border-2 relative group transition-all ${isQWrong ? 'bg-rose-50/30 border-rose-200' : isQCorrect ? 'bg-emerald-50/20 border-emerald-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                                                <div className="absolute top-4 left-4">
                                                    {isQCorrect && <div className="text-[10px] font-black text-emerald-600 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded-full scale-105 shadow-sm">LÀM ĐÚNG</div>}
                                                ...
                                                <div className="flex flex-col lg:flex-row gap-6 mt-6">
                                                    ...
                                                    <div className="flex-1 flex flex-col">
                                                        <div className="flex gap-4 mb-5">
                                                            <span className={`w-8 h-8 rounded-full flex flex-shrink-0 items-center justify-center font-bold text-white shadow-sm ${isQCorrect ? 'bg-emerald-500' : isQWrong ? 'bg-rose-500' : 'bg-slate-300'}`}>
                                                                {qNum}
                                                            </span>
                                                        ...
                                                        <div className="flex flex-col gap-3 mt-4">
                                                            {['A', 'B', 'C', 'D'].map((opt) => {
                                                                ...
                                                                if (isCorrectOpt) {
                                                                    optColorClass = 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-500/20 shadow-sm opacity-100';
                                                                ...
                                                                if (isCorrectOpt) badgeClass = 'border-emerald-500 bg-emerald-500 text-white shadow-sm';
                                                                    ...
                                                                    <div key={opt} className={`flex justify-between items-center p-3.5 rounded-xl border transition-all ${optColorClass}`}>
                                                                        <div className="flex items-center gap-3">
                                                                            ...
                                                                            {partInfo.part !== 1 && partInfo.part !== 2 ? (
                                                                                <span className={`font-medium ${isCorrectOpt ? 'text-emerald-800 font-bold' : (isChosen ? 'text-rose-700 line-through' : 'text-slate-600')}`} dangerouslySetInnerHTML={{ __html: q[`option${opt}`] }} />
                                                                            ) : (
                                                                                <span className={`font-medium ${isCorrectOpt ? 'text-emerald-800 font-bold' : (isChosen ? 'text-rose-700 line-through' : 'text-slate-600')}`}>Option {opt}</span>
                                                                            )}
                                                                        </div>
                                                                        ...
                                                                        {isCorrectOpt && <span className="text-[10px] font-black text-emerald-700 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded uppercase">Valid Answer</span>}
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                                        ...
                                                        {(q.explanation || q.translation) && (
                                                            <div className="mt-8 bg-sky-50/70 border-l-4 border-l-sky-400 border border-sky-100 rounded-lg p-5 shadow-sm">
                                                                <h4 className="font-bold text-sky-800 mb-3 flex items-center gap-2 text-sm uppercase tracking-wide">
                    ...
                    <div className={`absolute lg:relative right-0 top-0 bottom-0 z-50 transform transition-transform duration-300 w-[340px] max-w-[90vw] bg-white border-l border-slate-200 p-4 flex flex-col shadow-2xl lg:shadow-none ${isSheetOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
                        ...
                        <h3 className="font-bold text-lg mb-2 text-center text-indigo-900 border-b pb-3">Bảng Check Đáp Án</h3>
                        <div className="flex gap-4 justify-center py-2 text-[10px] font-bold text-slate-500 bg-slate-50 rounded-lg mb-4">
                            <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500"></span> Đúng</div>
                                        ...
                                        <a href={`#q-${qNum}`} onClick={() => {if(window.innerWidth < 1024) setIsSheetOpen(false)}} key={i} className={`flex items-center justify-between px-2 py-1.5 rounded-lg border text-sm transition-all ${isEnabled ? 'bg-slate-50 hover:border-indigo-300' : 'opacity-20 pointer-events-none grayscale'}`}>
                                            ...
                                            <div className="flex gap-1 shrink-0">
                                                {['A', 'B', 'C', 'D'].map(opt => {
                                                    ...
                                                    if (isCorrectOpt) badgeStyle = 'border-emerald-500 bg-emerald-500 text-white font-bold scale-110';
