    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <button 
                type="button"
                onClick={togglePlay}
                className="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-all shadow-sm active:scale-95"
            >
                {isPlaying ? (
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-800">
                <div className="animate-pulse font-bold text-xl text-primary-600">Đang tải dữ liệu Review...</div>
            </div>
        );
    }
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-slate-50 text-slate-800">
            <div className="flex flex-col h-screen overflow-hidden">
                {/* Header */}
                <div className="flex flex-row items-center justify-between px-2 py-3 md:p-4 gap-1 md:gap-4 bg-white shadow-sm border-b border-primary-100">
                    <div className="flex items-center gap-1.5 md:gap-4 flex-shrink-0">
                        <h1 className="font-black text-sm md:text-xl max-w-[200px] md:max-w-none truncate text-primary-900" title={testData.title}>REVIEW: {testData.title}</h1>
                        <span className="px-1.5 md:px-3 py-0.5 md:py-1 rounded-full text-[9px] md:text-xs font-bold whitespace-nowrap bg-primary-100 text-primary-700">
                            XEM CHỮA LỖI
                        </span>
                    </div>

                    <div className="flex items-center gap-1.5 md:gap-4 flex-1 justify-end">
                        <button onClick={() => setIsSheetOpen(!isSheetOpen)} className="lg:hidden px-2 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-sm font-bold flex items-center gap-1 md:gap-2 whitespace-nowrap flex-shrink-0 bg-primary-100 text-primary-700">
                ...
                <div className="flex flex-1 overflow-hidden">
                        ...
                        {testData.parts.map((partInfo: any, pIdx: number) => {
                            ...
                            return (
                                <div key={pIdx} id={`part-${partInfo.part}`} className="mb-12">
                                    <div className="bg-primary-50 border-l-4 border-primary-500 p-4 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center">
                                        <h2 className="text-xl font-black text-primary-900 mb-2 md:mb-0">Part {partInfo.part}</h2>
                                    ...
                                    <div className="space-y-10">
                                        {partInfo.questions.map((q: any, qIdx: number) => {
                                            ...
                                            return (
                                            <div key={q.id} id={`q-${qNum}`} className={`p-6 rounded-2xl border-2 relative group transition-all ${isQWrong ? 'bg-rose-50/30 border-rose-200' : isQCorrect ? 'bg-primary-50/20 border-primary-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                                                <div className="absolute top-4 left-4">
                                                    {isQCorrect && <div className="text-[10px] font-black text-primary-600 bg-primary-100 border border-primary-200 px-2 py-0.5 rounded-full scale-105 shadow-sm">LÀM ĐÚNG</div>}
                                                ...
                                                <button
                                                    ...
                                                    className={`absolute top-4 right-4 py-1.5 px-2.5 rounded-xl transition-all z-10 border flex items-center justify-center gap-1.5 ${bookmarkedIds[q.id] ? 'bg-secondary-50 border-secondary-300 text-secondary-600 shadow-sm' : 'bg-white border-secondary-200 text-secondary-500 hover:bg-secondary-50'}`}
                                                    title={bookmarkedIds[q.id] ? "Đã lưu" : "Bookmark"}
                                                >
                                                    <svg className={`w-5 h-5 ${bookmarkedIds[q.id] ? 'fill-secondary-500' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" /></svg>
                                                    ...
                                                    <div className="flex-1 min-w-0 flex flex-col">
                                                        ...
                                                        <div className="flex flex-col gap-3 mt-4">
                                                            {['A', 'B', 'C', 'D'].map((opt) => {
                                                                ...
                                                                if (isCorrectOpt) {
                                                                    optColorClass = 'border-primary-500 bg-primary-50 ring-2 ring-primary-500/20 shadow-sm opacity-100';
                                                                ...
                                                                if (isCorrectOpt) badgeClass = 'border-primary-500 bg-primary-500 text-white shadow-sm';
                                                                    ...
                                                                    <div key={opt} className={`flex justify-between items-center p-3.5 rounded-xl border transition-all ${optColorClass}`}>
                                                                        <div className="flex items-center gap-3">
                                                                            ...
                                                                            {partInfo.part !== 1 && partInfo.part !== 2 ? (
                                                                                <span className={`font-medium ${isCorrectOpt ? 'text-primary-800 font-bold' : (isChosen ? 'text-rose-700 line-through' : 'text-slate-600')}`} dangerouslySetInnerHTML={{ __html: q[`option${opt}`] }} />
                                                                            ) : (
                                                                                <span className={`font-medium ${isCorrectOpt ? 'text-primary-800 font-bold' : (isChosen ? 'text-rose-700 line-through' : 'text-slate-600')}`}>Option {opt}</span>
                                                                            )}
                                                                        </div>
                                                                        ...
                                                                        {isCorrectOpt && <span className="text-[10px] font-black text-primary-700 bg-primary-100 border border-primary-200 px-2 py-0.5 rounded uppercase">Valid Answer</span>}
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                                        ...
                                                        {(q.explanation || q.translation) && (
                                                            <div className="mt-8 bg-primary-50/70 border-l-4 border-l-primary-400 border border-primary-100 rounded-lg p-5 shadow-sm">
                                                                <h4 className="font-bold text-primary-800 mb-3 flex items-center gap-2 text-sm uppercase tracking-wide">
                                                                    <svg className="w-5 h-5 text-primary-500" fill="currentColor" viewBox="0 0 20 20"><path d="M11 3a1 1 0 10-2 0v1a1 1 0 102 0V3zM15.657 5.757a1 1 0 00-1.414-1.414l-.707.707a1 1 0 001.414 1.414l.707-.707zM18 10a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zM5 10a1 1 0 01-1 1H3a1 1 0 110-2h1a1 1 0 011 1zM8 16v-1h4v1a2 2 0 11-4 0zM12 14c.015-.34.208-.646.477-.859a4 4 0 10-4.954 0c.27.213.462.519.476.859h4.002z" /></svg>
                                                                    Giải Thích Chi Tiết
                                                                </h4>
                    ...
                    <div className={`absolute lg:relative right-0 top-0 bottom-0 z-50 transform transition-transform duration-300 w-[340px] max-w-[90vw] bg-white border-l border-slate-200 p-4 flex flex-col shadow-2xl lg:shadow-none ${isSheetOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
                        ...
                        <h3 className="font-bold text-lg mb-2 text-center text-primary-900 border-b pb-3">Bảng Check Đáp Án</h3>
                        <div className="flex gap-4 justify-center py-2 text-[10px] font-bold text-slate-500 bg-slate-50 rounded-lg mb-4">
                            <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-primary-500"></span> Đúng</div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                            <div className="grid grid-cols-2 gap-2">
                                {Array.from({length: 200}).map((_, i) => {
                                    ...
                                    return (
                                        <a href={`#q-${qNum}`} onClick={() => {if(window.innerWidth < 1024) setIsSheetOpen(false)}} key={i} className={`flex items-center justify-between px-2 py-1.5 rounded-lg border text-sm transition-all ${isEnabled ? 'bg-slate-50 hover:border-primary-300' : 'opacity-20 pointer-events-none grayscale'}`}>
                                            <span className={`font-bold w-6 text-right mr-1 sm:mr-1.5 ${isCorrect ? 'text-primary-500' : isIncorrectOrBlank ? 'text-rose-500' : 'text-slate-400'}`}>{qNum}.</span>
                                            <div className="flex gap-1 shrink-0">
                                                {['A', 'B', 'C', 'D'].map(opt => {
                                                    ...
                                                    if (isCorrectOpt) badgeStyle = 'border-primary-500 bg-primary-500 text-white font-bold scale-110';
