...
export default function NotebookGenericBank({ type, filter, partFilter }: { type: string, filter: string, partFilter?: string }) {
    ...
    if (isHistory && records.length === 0) {
        return (
			<div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 text-center shadow-sm">
				<div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full flex items-center justify-center text-2xl mx-auto mb-4 border border-slate-100 dark:border-slate-700">
					<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
				</div>
				<h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Chưa có lịch sử thi</h3>
    ...
    if (isHistory) {
        return (
            <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 text-center shadow-sm">
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Lịch sử thi (Tóm tắt)</h3>
                <p className="text-slate-500 dark:text-slate-400 mb-4">Bạn có {records.length} bản ghi thi thử.</p>
        ...
        <div className="space-y-4 pb-10">
            ...
            {items.map((item: any) => {
                ...
                const wrapperProps = type === 'ACTUAL_TEST' 
                    ? { key: q.id, className: "block bg-white dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700 transition-all group relative cursor-default" }
                    : { key: q.id, href: `/toeic-practice/${typePath}/${q.lesson.topic.slug}?lessonId=${q.lesson.id}&reviewId=${q.id}`, className: "block bg-white dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700 transition-all group relative cursor-pointer" };
                ...
                return (
                    <Wrapper {...wrapperProps as any}>
                        ...
                        <div className="mb-4 pr-8">
                            {q.imageUrl && (
                                <ZoomableImage src={q.imageUrl} className="w-[150px] object-cover rounded shadow-sm border border-slate-200 dark:border-slate-700 mb-3" />
                            )}
                            {q.audioUrl && (
                                <audio src={q.audioUrl} controls className="h-10 w-full max-w-sm mb-3" />
                            )}
                            <p className="text-lg font-black text-slate-800 dark:text-slate-200 line-clamp-2 leading-snug">
                                {q.question}
                            </p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4 text-sm font-medium">
                            <div className={`p-2.5 rounded-lg border flex flex-col gap-1.5 ${q.correctOption === 'A' ? 'bg-primary-50 dark:bg-primary-900/30 border-primary-200 dark:border-primary-800 text-primary-800 dark:text-primary-300' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400'}`}>
                            ...
                            <div className={`p-2.5 rounded-lg border flex flex-col gap-1.5 ${q.correctOption === 'B' ? 'bg-primary-50 dark:bg-primary-900/30 border-primary-200 dark:border-primary-800 text-primary-800 dark:text-primary-300' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400'}`}>
                            ...
                            <div className={`p-2.5 rounded-lg border flex flex-col gap-1.5 ${q.correctOption === 'C' ? 'bg-primary-50 dark:bg-primary-900/30 border-primary-200 dark:border-primary-800 text-primary-800 dark:text-primary-300' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400'}`}>
                            ...
                            {q.optionD && (
                            <div className={`p-2.5 rounded-lg border flex flex-col gap-1.5 ${q.correctOption === 'D' ? 'bg-primary-50 dark:bg-primary-900/30 border-primary-200 dark:border-primary-800 text-primary-800 dark:text-primary-300' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400'}`}>
                        ...
                        {(q.explanation) && (
                            <div className="mt-4 pt-3 border-t border-slate-100/60">
                                {hideTranscript(q.explanation) && (
                                    <div className="text-[13px] font-medium text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{hideTranscript(q.explanation)}</div>
                                )}
                            </div>
                        )}
                    </Wrapper>
