...
/** Return the longest consecutive-day streak from a list of date strings (YYYY-MM-DD). */
...
export async function GET() {
  ...
  // Fetch raw activity data in parallel
  ...
  const onTimeSubmissions = submissions.filter(
    (s) => s.submittedAt <= s.homework.dueDate
  ).length
  ...
  const badges: BadgeItem[] = [
    ...
    },
    {
      ...
      earnedAt: onTimeSubmissions >= 3 ? submissions.filter((s) => s.submittedAt <= s.homework.dueDate)[2]?.createdAt.toISOString() ?? null : null,
