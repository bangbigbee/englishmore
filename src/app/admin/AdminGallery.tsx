...
export default function AdminGallery() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <select
            ...
            className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-900 focus:outline-none focus:ring-1 focus:ring-primary-900"
          >
          ...
          <button
            ...
            disabled={uploading}
            className="whitespace-nowrap rounded-lg bg-primary-900 px-4 py-2 font-semibold text-white hover:bg-primary-800 disabled:opacity-50"
          >
      ...
      {selectedCourseId === '' && (
        <div className="rounded-lg border border-primary-100 bg-primary-50 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="font-semibold text-primary-900">Animation Style</h3>
            <p className="text-xs text-primary-700">Chọn cách thức hiển thị ảnh động cho Hình ảnh chung trên trang chủ.</p>
          </div>
          <select
            value={generalAnimation}
            onChange={(e) => handleUpdateGeneralAnimation(e.target.value)}
            className="rounded bg-white border border-primary-300 px-3 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-primary-500"
          >
      ...
      {selectedCourseId && selectedCourseId !== 'teacher' && selectedCourseDetails && (
        <div className="rounded-lg border border-primary-100 bg-primary-50 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="font-semibold text-primary-900">Animation Style</h3>
            <p className="text-xs text-primary-700">Chọn cách thức hiển thị ảnh động cho khóa học này trên trang chủ.</p>
          </div>
          <select
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-1 text-xs cursor-pointer">
                  <input
                    ...
                    className="h-3 w-3 rounded border-gray-300 text-primary-900 focus:ring-primary-900"
                  />
                  Active
                </label>
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    className="w-12 rounded border border-gray-300 px-1 py-0.5 text-xs focus:border-primary-900 focus:outline-none focus:ring-1 focus:ring-primary-900"
                  />
                </div>
              </div>
              <button
