                    ...
                    {SUB_TABS.map(tab => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all shrink-0 whitespace-nowrap ${
                                subTab === tab.id
                                    ? 'bg-white text-primary-900 shadow-sm xl:bg-primary-900 xl:text-white'
        ...
        <div className="mt-8 animate-in fade-in zoom-in-95 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
				<div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-center relative overflow-hidden shadow-sm">
					<div className="absolute -right-6 -top-6 w-24 h-24 bg-primary-50 rounded-full blur-2xl"></div>
					...
					<div className="mt-4 w-full bg-slate-100 rounded-full h-2">
						<div className="bg-primary-500 h-2 rounded-full" style={{ width: `${Math.min(100, completionRate)}%` }} />
					</div>
					<p className="text-xs font-bold text-primary-600 mt-2">Hoàn thành {completionRate}% mục tiêu</p>
				</div>
				...
				<div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-center relative overflow-hidden shadow-sm">
					<div className="absolute -right-6 -top-6 w-24 h-24 bg-secondary-50 rounded-full blur-2xl"></div>
					...
					<div className="mt-4 flex items-center gap-2">
						<span className="flex items-center justify-center w-5 h-5 rounded-md bg-secondary-100 text-secondary-600 text-xs shadow-sm">⭐</span>
            ...
            {/* Heatmap Section */}
			<VocabularyHeatmap heatmap={heatmap} />
            
            <div className="bg-primary-50 text-primary-800 rounded-2xl p-6 border border-primary-100 flex gap-4 mt-6">
        ...
        <div className="mt-8 animate-in fade-in zoom-in-95 duration-300">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-center relative overflow-hidden shadow-sm">
					...
					<div className="mt-4 w-full bg-slate-100 rounded-full h-2">
						<div className="bg-primary-500 h-2 rounded-full" style={{ width: hasData ? `${Math.round((stats.correct / (stats.correct + stats.incorrect)) * 100)}%` : '0%' }} />
					</div>
				</div>
				...
				<div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-center relative overflow-hidden shadow-sm">
					...
					<div className="mt-4 flex items-center gap-2">
						<span className="flex items-center justify-center w-5 h-5 rounded-md bg-primary-100 text-primary-600 text-xs shadow-sm">⏱️</span>
