            ...
            <div className="flex flex-nowrap overflow-x-auto gap-2 bg-slate-100/80 p-1.5 rounded-xl w-full sm:w-fit border border-slate-200/50 custom-scrollbar pb-2.5 sm:pb-1.5">
                ...
                {showHistory && (
                    <button 
                        ...
                        className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-lg transition-all duration-300 shrink-0 whitespace-nowrap ${filter === 'history' ? 'bg-white text-primary-600 shadow-sm ring-1 ring-slate-900/5' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'}`}
                    >
            ...
            <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-xl border border-slate-200 select-none">
                ...
                {[1, 2, 3, 4, 5, 6, 7].map(partNum => (
                    <label key={partNum} className="flex items-center gap-2 cursor-pointer group">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${currentParts.includes(partNum) ? 'bg-primary-900 border-primary-900' : 'border-slate-300 bg-white group-hover:border-primary-900/50'}`}>
