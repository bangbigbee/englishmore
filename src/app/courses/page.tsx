    ...
    <div className="flex flex-col min-h-screen lg:h-[calc(100vh-84px)] lg:min-h-0 bg-gray-50 lg:overflow-hidden">
      <div className="bg-white shadow-xs border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-primary-900">Available Courses</h1>
      ...
      <div className="flex-1 lg:min-h-0 mx-auto max-w-7xl w-full px-4 py-8 sm:px-6 lg:px-8">
        {loading ? (
          ...
          <div className="lg:h-full lg:grid lg:grid-cols-[320px_1fr] lg:gap-8">
            {/* Sidebar - Course List */}
            <aside className="relative z-10 mb-8 lg:mb-0 lg:h-full lg:overflow-y-auto custom-scrollbar lg:px-2">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left transition-all duration-300 rounded-xl border p-4 shadow-sm hover:shadow-md ${
                        isActive 
                          ? 'border-primary-900 bg-linear-to-br from-primary-900/10 to-white' 
                          : 'border-slate-200 bg-white hover:border-primary-900/30'
                      }`}
                    >
                      <p className={`font-bold leading-tight ${isActive ? 'text-primary-900' : 'text-slate-800'}`}>
                        {course.title}
                      </p>
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          isFull ? 'bg-red-50 text-red-600' : 'bg-primary-50 text-primary-600'
                        }`}>
                    ...
                    <div className="rounded-[calc(1rem-1.5px)] overflow-hidden bg-white">
                      <div className="px-5 py-6 sm:px-8 sm:py-8 border-b border-slate-100">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                          <div>
                            <h2 className="text-3xl font-extrabold text-primary-900 leading-tight">{course.title}</h2>
                            <div className="mt-2 flex flex-wrap items-center gap-4 text-sm">
                              <p className="text-slate-600">
                                <strong>Hạn đăng ký:</strong> {registrationDeadlineText}
                              </p>
                              <p className={`font-bold ${isFull ? 'text-red-700' : 'text-primary-700'}`}>
                        ...
                        <div className="mt-8">
                          <button
                            ...
                            className="w-full sm:w-auto rounded-xl bg-primary-900 px-10 py-4 text-base font-bold text-white shadow-lg transition hover:bg-primary-800 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"
                          >
                            {registering === course.id ? 'Đang đăng ký...' : 'Đăng Ký Ngay'}
                          </button>
                          
                          {isPendingPayment && (
                            <p className="mt-4 text-sm font-semibold text-primary-700 bg-primary-50 border border-primary-200 p-4 rounded-lg">
                        ...
                        {enrollment && enrollment.status !== 'pending' && (
                          <div className="mt-6 rounded-xl border border-primary-200 bg-primary-50 p-4 text-sm text-primary-800">
                            ...
                            <button
                              ...
                              className="mt-3 inline-block rounded-lg border border-primary-900/30 bg-white px-4 py-2 text-xs font-bold text-primary-900 hover:bg-primary-900/10"
                            >
                      ...
                      <div className="px-5 pb-8 pt-4 sm:px-8 sm:pb-10 sm:pt-5 space-y-8">
                                    ...
                                    {phases.map((phase) => (
                                      <div 
                                        ...
                                        className={`flex-1 rounded-2xl border-2 p-4 transition-all duration-300 ${
                                          phase.isActive 
                                            ? phase.color === 'emerald' ? 'border-primary-500 bg-primary-50 ring-4 ring-primary-500/10 scale-[1.02]' :
                                              phase.color === 'blue' ? 'border-primary-500 bg-primary-50 ring-4 ring-primary-500/10 scale-[1.02]' :
                                        ...
                                        <div className="flex flex-col h-full justify-between">
                                          <div>
                                            <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider mb-2 ${
                                              phase.isActive 
                                                ? phase.color === 'emerald' ? 'bg-primary-500 text-white' :
                                                  phase.color === 'blue' ? 'bg-primary-500 text-white' :
                                          ...
                                          <p className={`mt-3 text-lg font-black ${
                                            phase.isActive 
                                              ? phase.color === 'emerald' ? 'text-primary-700' :
                                                phase.color === 'blue' ? 'text-primary-700' :
                                                'text-slate-700'
                                              : 'text-slate-400'
                                          }`}>
                                  ...
                                  {tier.name !== 'Regular' && (
                                    <div className="rounded-xl bg-primary-50 border border-primary-100 p-3 flex items-center justify-between">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-sm font-bold text-primary-800">Giai đoạn hiện tại: {tier.name}</p>
                                      </div>
                        ...
                        <div className="pt-6 border-t border-slate-100">
                          <p className="text-sm text-slate-600 leading-relaxed">
                            Mọi thông tin thêm, vui lòng liên hệ <strong>Mr. Nguyễn Trí Bằng</strong> qua số điện thoại <a href="tel:0915091093" className="text-primary-900 hover:underline font-semibold">0915091093</a>. 
                            Hoặc nhắn tin về Facebook:
                            <a href="https://www.facebook.com/bangbigbee" target="_blank" rel="noreferrer" className="block mt-2 font-medium text-primary-900 hover:underline">facebook.com/bangbigbee</a>
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          {paymentInstruction && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <motion.div 
                ...
                className="rounded-lg border border-primary-900/40 bg-white shadow-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto"
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-primary-900">Payment Details</h3>
                  <button
                    onClick={() => {
                      setPaymentInstruction(null)
                      setPendingRegistrationDraft(null)
                ...
                <p className="text-primary-700 bg-primary-50 border border-primary-200 rounded p-3 text-sm mb-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="py-2">
                    ...
                    <div className="rounded border-2 border-primary-900 bg-primary-50 p-3 text-center">
                      <span className="font-mono text-sm font-normal tracking-wider text-primary-900">
                        <LinkifiedText text={paymentInstruction.transferContent} preserveLineBreaks={false} linkClassName="break-all font-medium text-primary-900 underline underline-offset-2 hover:text-primary-800" />
                      </span>
                    </div>
                    <p className="text-sm text-primary-900 font-semibold mt-1">Example: Nguyen Van A - 0934567890</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="mt-4 w-full px-4 py-2 bg-primary-900 text-white rounded hover:bg-primary-800"
                >
          ...
          {pendingReferralCourse && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                exit={{ opacity: 0, scale: 0.95, y: 10 }}
                transition={{ type: "spring", damping: 25, stiffness: 300, duration: 0.2 }}
                className="relative w-full max-w-md rounded-lg border-2 border-primary-900 bg-white p-6 shadow-[0_18px_48px_rgba(88, 28, 135,0.22)]"
              >
                <div className="mb-4 flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-xl font-bold text-primary-900">Ai đã giới thiệu khóa học {pendingReferralCourse.title} này cho bạn?</h3>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="text-2xl leading-none text-primary-900/30 hover:text-primary-900"
                  >
                    ×
                  </button>
                </div>
                ...
                <input
                  ...
                  className="mt-2 block w-full rounded-lg border border-slate-200 px-4 py-2 outline-none focus:border-primary-900 focus:ring-1 focus:ring-primary-900"
                />
                ...
                <div className="mt-6 flex justify-end gap-3">
                  ...
                  <button
                    ...
                    className="rounded bg-primary-900 px-4 py-2 text-sm font-medium text-white hover:bg-primary-800 disabled:opacity-50"
                  >
...
export default function CoursesPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-white"><div className="h-8 w-8 animate-spin rounded-full border-4 border-primary-900 border-t-transparent"></div></div>}>
      <CoursesContent />
    </Suspense>
  )
}
