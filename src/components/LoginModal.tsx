...
export default function LoginModal({ isOpen, onClose, callbackUrl = '/' }: LoginModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 isolate">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-8 shadow-2xl transition-all duration-300 animate-in zoom-in-95 fade-in slide-in-from-bottom-4">
        {/* Close Button */}
        <button
          onClick={onClose}
